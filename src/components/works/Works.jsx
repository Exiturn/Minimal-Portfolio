...
const Works = () => {
    ...
    return (
        <div id='Works' className='flex flex-col justify-start items-start px-5 md:px-[10vw] min-w-[100vw] min-h-[120vh] md:min-h-[100vh] overflow-hidden relative'>
            <h1 className='text-[3rem] font-medium flex justify-center items-center'>Works</h1>
                    ...
                    {works.map((work, index) => (
                        <div className={`${slide === index ? 'flex flex-col justify-start items-start project w-full md:w-[60vw]' : 'md:hidden'} transition ease-in-out bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg z-0 px-4 pb-4`} key={index}>
                            <div className='flex flex-row w-full justify-between'>
                                <h1 className='text-[2.5rem] font-medium text-start'>{work.title}</h1>
                            ...
                            <button onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} className='mt-4 font-medium underline underline-offset-2'>More Details</button>
                        </div>
                    ))}
                </div>
            </div>
